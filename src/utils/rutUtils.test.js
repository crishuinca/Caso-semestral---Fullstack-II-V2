...
describe('RUT Utils', () => {
  describe('validarRUT', () => {
    test('PRUEBA_01: Valida RUT correcto', () => {
      ...
      expect(validarRUT('1234567-8')).toBe(true);
    });
  ...
  describe('calcularDigitoVerificador', () => {
    test('PRUEBA_07: Calcula dígito verificador correcto', () => {
      expect(calcularDigitoVerificador('12345678')).toBe('9');
  ...
  describe('formatearRUT', () => {
    test('PRUEBA_12: Formatea RUT con dígito verificador', () => {
      expect(formatearRUT('12345678-9')).toBe('12345678-9');
      expect(formatearRUT('1234567-K')).toBe('1234567-K');
    });

    test('PRUEBA_13: Agrega dígito verificador si falta', () => {
      expect(formatearRUT('12345678')).toBe('12345678-9');
      expect(formatearRUT('1234567')).toBe('1234567-K');
    });

    test('PRUEBA_14: Limpia caracteres no válidos', () => {
      expect(formatearRUT('12.345.678-9')).toBe('12345678-9');
      expect(formatearRUT('12 345 678-9')).toBe('12345678-9');
    });
  ...
  describe('formatearRUTEnTiempoReal', () => {
    ...
    test('PRUEBA_17: Limita caracteres', () => {
      expect(formatearRUTEnTiempoReal('12345678901')).toBe('12345678-9');
    });
  ...
  describe('obtenerMensajeErrorRUT', () => {
    ...
    test('PRUEBA_24: Retorna mensaje para RUT sin guión', () => {
      expect(obtenerMensajeErrorRUT('12345678')).toBe('El RUT debe incluir un guión separador');
    });
    ...
    test('PRUEBA_27: Retorna string vacío para RUT válido', () => {
      expect(obtenerMensajeErrorRUT('12345678-9')).toBe('');
      expect(obtenerMensajeErrorRUT('11111111-1')).toBe('');
    });

    test('PRUEBA_28: Maneja RUT con K', () => {
      expect(obtenerMensajeErrorRUT('1234567-K')).toBe('');
      expect(obtenerMensajeErrorRUT('1234567-k')).toBe('');
    });
  });
});
